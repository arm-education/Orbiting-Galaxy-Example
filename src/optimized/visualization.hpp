...
public:
    ...
    void dump_frame(const ParticlesSoA& particles) {
        if (!enabled_ || !fp_.is_open()) return;

        for (int j = 0; j < num_particles_; j += vis_stride_)
            fp_.write(reinterpret_cast<const char*>(&particles.x[j]), sizeof(float));
        for (int j = 0; j < num_particles_; j += vis_stride_)
            fp_.write(reinterpret_cast<const char*>(&particles.y[j]), sizeof(float));
        for (int j = 0; j < num_particles_; j += vis_stride_)
            fp_.write(reinterpret_cast<const char*>(&particles.z[j]), sizeof(float));
    }
